...
TMap<FString, UDBTweenBase*> UDBTweenBase::DBTweenDic;
...
{
	if (!DBTweenDic.Contains(DBTweenKey) || DBTweenKey.IsEmpty())
	{
		return false;
	}
	return true;
...
void UDBTweenBase::Init(float durTime)
{
	...
	DBTweenDic.Add(MyDBTweenKey, this);
}
...
{
	if (DBTweenDic.Contains(TweenName) || TweenName.IsEmpty())
	{
		MyDBTweenKey = FString::FromInt(this->GetUniqueID());
	}
	else
